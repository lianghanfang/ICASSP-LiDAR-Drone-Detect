...
class XYZConverter:
    ...
    def _convert_to_xyz(self, folder_name, subfolder_name):
        ...
        output_folder = os.path.join(self.output_file_path, str(self.sequence), subfolder_name)
    ...
    def convert_livox_avia(self):
        self._convert_to_xyz("livox_avia", "livox_avia_xyz")

    def filter_lidar_360(self, height_threshold=0.7):
    ...
    #     for file in file_list:
    #         data_array = np.load(os.path.join(folder_path, file))
    #         # print(f"Processing {file} with shape {data_array.shape}")
    #         filename = os.path.splitext(file)[0] + ".xyz"
    #         output_file = os.path.join(output_folder, filename)
    #         self._write_xyz_file(data_array, output_file)
    #
...
# converter = XYZConverter(root_path="path_to_root", output_file_path="path_to_output", sequence=1)
...
# sequence = 21
# root_path = r"K:\梁\Anti-UAV\MMUAV_dataset\Anti_UAV_data\train\seq" + str(sequence)
# output_file_path = r"E:\CCFA\project\dataset\MMAUD"
